...
export class MihomeVacuum extends utils.Adapter {
    constructor(options = {}) {
        super({
            ...options,
            name: 'mihome-vacuum',
    ...
    async getTimersForAdmin() {
        ...
        const rooms = (Object.values(roomObjects || {}) as RoomEnumObject[]).map(room => ({
            id: room._id,
            name: room.common.name,
            members: room.common.members || [],
        }));
        const channels = (Object.values(objects) as AdapterRuntimeObject[])
        ...
        const timers = (Object.values(objects) as AdapterRuntimeObject[])
    ...
    async saveTimersFromAdmin(timers) {
        ...
        const prefix = `${this.namespace}.timer.`;
        for (const object of Object.values(existing) as AdapterRuntimeObject[]) {
            if (
                object.type === 'state' &&
        ...
        for (const room of Object.values(roomObjects || {}) as RoomEnumObject[]) {
