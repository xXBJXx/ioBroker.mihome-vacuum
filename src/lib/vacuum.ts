...
class VacuumManager {
    // The manager remains the compatibility boundary for legacy ioBroker messages while its extracted
    // protocol and helper modules provide the strict external-data contracts.
    [property: string]: any;
    ...
    async getLogEntries(logArray) {
        ...
        const cleanJSON: DisplayCleaningRecord[] = [];
    ...
    async setGetConsumable() {
        try {
            ...
            if (!this.features.consumables) {
                ...
                const detectedConsumables = consumableProtocol.detectConsumables(
                    consumable,
                    objects.stockConsumable.list,
                    commands,
                );
                for (const detected of detectedConsumables) {
                    let contents = await this.adapter.setObjectNotExistsAsync(
                        `consumable.${detected.state._id}`,
                        detected.state,
                    );
                    contents && this.adapter.log.debug(`Create State for consumable: ${JSON.stringify(contents)}`);
                    contents = await this.adapter.setObjectNotExistsAsync(
                        `consumable.${detected.button._id}`,
                        detected.button,
                    );
        ...
        if (obj) {
            ...
            switch (obj.command) {
                ...
                case 'move': {
                    ...
                    return await this.Miio.sendMessage(
                        'app_rc_move',
                        commandPayloads.createRemoteMovePayload(obj.message),
                    );
                }
                // ======================================================================
                default:
