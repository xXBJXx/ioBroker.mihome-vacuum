...
class Miio extends EventEmitter {
    constructor(adapterInstance) {
        ...
        this.timeout = null;
        /** @type {Map<number, {resolve: (value: object) => void, reject: (reason: Error) => void, method: string, startedAt: number}>} */
    ...
    _createRequestError(code, message) {
        return Object.assign(new Error(message), { code });
    }

    _finishRequest(msgCounter, settle, result) {
        const request = this.pendingRequests.get(msgCounter);
        if (!request) {
            return;
        }
    ...
    _failRequest(msgCounter, code, message) {
        this._finishRequest(
            msgCounter,
            (request, error) => request.reject(error),
            this._createRequestError(code, message),
        );
    }

    _failAllPending(code, message) {
        for (const msgCounter of [...this.pendingRequests.keys()]) {
            this._failRequest(msgCounter, code, message);
        }
    }
    ...
    _handleMessage(msg, rinfo) {
        ...
        const request = this.pendingRequests.get(answer.id);
        if (!request) {
            return;
        }
        ...
        this.adapter.log.debug(
            `MIIO request succeeded: method=${request.method}, id=${answer.id}, duration=${Date.now() - request.startedAt}ms`,
        );
        this._finishRequest(answer.id, (pendingRequest, result) => pendingRequest.resolve(result), answer);
    }
    ...
    async sendMessage(method, params) {
        ...
        return new Promise((resolve, reject) => {
            if (this.closing) {
                return reject(this._createRequestError('MIIO_CLOSED', 'MIIO client is closed'));
            }
            if (!this.connected) {
                this.adapter.log.debug('your device is not connected, but this could be temporary');
                return reject(this._createRequestError('MIIO_NOT_CONNECTED', 'MIIO device is not connected'));
            }
            ...
            try {
                ...
                return reject(this._createRequestError('MIIO_REQUEST_FAILED', 'MIIO request failed'));
            }
            this.pendingRequests.set(msgCounter, { resolve, reject, method: requestMethod, startedAt });
            ...
            try {
                this.server.send(rawMsg, 0, rawMsg.length, parseInt(this.port), this.adapter.config.ip, err => {
                    ...
                    if (err) {
                        ...
                        return this._failRequest(msgCounter, 'MIIO_SEND_FAILED', 'MIIO request could not be sent');
                    }
                    this.globalTimeouts[`sendMessage${msgCounter}`] = setTimeout(
                        cnt => {
                            ...
                            this._failRequest(msgCounter, 'MIIO_TIMEOUT', 'MIIO request timed out');
                        },
                        2000,
                        msgCounter,
                    );
                ...
                this.adapter.log.error(`MIIO request failed: method=${safeMethod}`);
                return this._failRequest(msgCounter, 'MIIO_REQUEST_FAILED', 'MIIO request failed');
            }
        });
    }
