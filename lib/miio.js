...
class Miio extends EventEmitter {
    ...
    async sendMessage(method, params) {
        const safeMethod = String(method).replace(/[^a-zA-Z0-9_.-]/g, '?').slice(0, 80);
        return new Promise((resolve, reject) => {
            ...
            this.adapter.log.debug(
                `MIIO request started: method=${requestMethod}, id=${msgCounter}, duration=0ms`,
            );
