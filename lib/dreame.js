    ...
    async init() {
        await Promise.all(
            objects.stockControl.map(async o => {
                if (!DreameBlockedObjects.includes(o._id)) {
                    const contents = await this.adapter.setObjectAsync(`control${o._id ? `.${o._id}` : ''}`, o);
        ...
        await Promise.all(
            objects.stockInfo.map(async o => {
                if (!DreameBlockedObjects.includes(o._id)) {
                    const contents = await this.adapter.setObjectAsync(`info${o._id ? `.${o._id}` : ''}`, o);
        ...
        await Promise.all(
            objects.settings.map(async o => {
                if (!DreameBlockedObjects.includes(o._id)) {
                    const contents = await this.adapter.setObjectAsync(`setting${o._id ? `.${o._id}` : ''}`, o);
        ...
        await Promise.all(
            objects.stockHistory.map(async o => {
                if (!DreameBlockedObjects.includes(o._id)) {
                    const contents = await this.adapter.setObjectAsync(`history${o._id ? `.${o._id}` : ''}`, o);
