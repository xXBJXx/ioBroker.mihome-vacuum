...
class DreameManager {
    constructor(adapterInstance, Miio) {
        ...
        this.Miio.sendMessage('get_properties', data)
            .then(result => {
                if (this.closed) {
                    return;
                }
        ...
        await Promise.all(
            objects.settings.map(async o => {
                if (!DreameBlockedObjects.includes(o._id)) {
                    ...
                    contents && this.adapter.log.debug(`Create State for settings: ${JSON.stringify(contents)}`);
                }
            }),
        );
            ...
            await Promise.all(
                objects.wash_base.map(async o => {
                    if (!DreameBlockedObjects.includes(o._id)) {
                        ...
                        contents &&
                            this.adapter.log.debug(`Create Wash Base State for control: ${JSON.stringify(contents)}`);
                    }
                }),
            );
            await Promise.all(
                objects.wash_base_info.map(async o => {
                    if (!DreameBlockedObjects.includes(o._id)) {
                        ...
                        contents &&
                            this.adapter.log.debug(`Create Wash Base State for info: ${JSON.stringify(contents)}`);
                    }
                }),
            );
        }
    ...
    async getStates() {
        ...
        for (let i = 0; i < this.PARAMS.length; i += chunkSize) {
            try {
                ...
                this.adapter.log.debug(`Requesting ${returnArray.length} Dreame properties`);
    ...
    updateObjectValue(property, control, element) {
        ...
        if (!this.getSpecialHandlingValues(property.control, value)) {
            ...
            this.adapter.setStateAsync(control, { val: value, ack: true });
    ...
    async stateChange(id, state) {
        ...
        try {
            ...
            if (DeviceData && DeviceData.result) {
                ...
                this.adapter.setStateAsync(id, { val: false, ack: true });
                this.getStates();
                return;
            }
        } catch {
    ...
    async washMop() {
        ...
        let state = await this.adapter.getStateAsync('info.dock_state');
...
DreameManager.DreameWaterVolumes = DreameWaterVolumes;
DreameManager.DreameErrors = DreameErrors;
DreameManager.DreameState = DreameState;
DreameManager.DreameWashBaseState = DreameWashBaseState;
DreameManager.DreameProperties = DreameProperties;
DreameManager.DreameActions = DreameActions;
DreameManager.DreameBlockedObjects = DreameBlockedObjects;
