...
class RoomManager {
    ...
    findChannelsByMapIndex(mapList, callback) {
        this.adapter.getStates('rooms.*', (err, states) => {
            ...
            if (states) {
                for (const stateId in states) {
                    if (
                        stateId.endsWith('.mapIndex') &&
                        states[stateId] &&
                        mapList.indexOf(states[stateId].val) >= 0
                    ) {
    ...
    createRoom(roomId, mapIndex) {
        ...
        this.adapter.createChannel('rooms', roomId, (err, roomObj) => {
            if (roomObj) {
                const commonZone = {
                    name: 'map zone',
                    type: 'string',
                ...
                this.adapter.setObjectNotExists(
                    ...
                    {
                        type: 'state',
                        common: mapIndex[0] === '[' ? commonZone : commonMap,
                        native: {},
                    },
                    err => !err && this.adapter.setState(`${roomObj.id}.mapIndex`, mapIndex, true),
                );
