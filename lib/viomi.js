...
class ViomiManager {
    ...
    async getStates() {
        ...
        if (DeviceData && Array.isArray(DeviceData.result)) {
            ...
            answer.slice(0, this.PARAMS.length).forEach((element, index) => {
                const objExist = objects.viomiObjects.find(stateDefinition => stateDefinition._id === this.PARAMS[index]);
        ...
        if (!this.closed) {
            this.globalTimeouts['getStates'] = setTimeout(
    ...
    async initStates() {
        objects.viomiObjects.map(o =>
            this.adapter.setObjectNotExistsAsync(`control${o._id ? `.${o._id}` : ''}`, o),
        );
    }
