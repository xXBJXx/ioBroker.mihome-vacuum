...
/** @type {Array<[string, typeof import('../build/lib/XiaomiCloudConnector')]>} */
const connectorImplementations = [
    // @ts-expect-error The checked legacy Axios shape differs nominally from the generated declaration.
    ['legacy JavaScript', require('./XiaomiCloudConnector')],
    ['TypeScript build', require('../build/lib/XiaomiCloudConnector')],
];
const { XiaomiRC4Cipher } = require('./XiaomiCloudCrypto');
...
for (const [implementationName, XiaomiCloudConnector] of connectorImplementations) {
    describe(`XiaomiCloudConnector QR login (${implementationName})`, () => {
        let originalCreate;
