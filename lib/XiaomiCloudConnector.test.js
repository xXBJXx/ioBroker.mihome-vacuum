...
describe('XiaomiCloudConnector QR login', () => {
    ...
    it('encrypts persisted session data before writing native configuration', async () => {
        ...
        const connector = new XiaomiCloudConnector(
            { debug() {}, info() {}, warn() {}, error() {} },
            { cloudSession: JSON.stringify({ ssecurity: 'YWJjZGVmZ2hpamtsbW5vcA==', userId: '42', serviceToken: 'token-value-123', location: 'https://login.example/token' }) },
            adapter,
        );
    ...
    it('follows Xiaomi redirects before reading the service token cookie', async () => {
        ...
        axios.create = () => ({
            get: async () => {
                requests++;
                return requests === 1
                    ? { status: 302, headers: { location: 'https://login.example/final', 'set-cookie': ['passToken=temporary; Path=/'] } }
