...
RRMapParser.PARSEBLOCK = function parseBlock(buf, offset, result) {
    ...
    switch (type) {
        ...
        case RRMapParser.TYPES.CHARGER_LOCATION:
            result[type] = {
                position: [buf.readUInt16LE(0x08 + offset), buf.readUInt16LE(0x0c + offset)],
        ...
        case RRMapParser.TYPES.IMAGE: {
            ...
            /**
             * @type {{
             *   segments: {count: number, id: number[]},
             *   position: {top: number, left: number},
             *   dimensions: {height: number, width: number},
             *   pixels: number[] | {floor: number[], obstacle: number[], segments: number[]}
             * }}
             */
            const parameters = {
                segments: {
                ...
                for (let s, i = 0; i < length; i++) {
                    switch (buf.readUInt8(0x18 + g3offset + offset + i) & 0x07) {
                        ...
                        default:
                            ...
                            if (s !== 0) {
                                if (!parameters.segments.id.includes(s)) {
                                    parameters.segments.id.push(s);
                                }
        ...
        case RRMapParser.TYPES.GOTO_PREDICTED_PATH: {
            ...
            for (let i = 0; i < length; i = i + 4) {
                ...
                points.push([buf.readUInt16LE(0x14 + offset + i), buf.readUInt16LE(0x14 + offset + i + 2)]);
            }
            result[type] = {
        ...
        case RRMapParser.TYPES.GOTO_TARGET:
            result[type] = {
                position: [buf.readUInt16LE(0x08 + offset), buf.readUInt16LE(0x0a + offset)],
            };
            break;
...
RRMapParser.PARSEDATA = function parseData(mapBuf) {
    ...
    if (blocks[RRMapParser.TYPES.IMAGE]) {
        //We need the image to flip everything else correctly
