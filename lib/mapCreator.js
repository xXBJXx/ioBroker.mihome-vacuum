...
let canvasImg = createCanvas(100, 100);
...
MapCreator.CanvasMap = (Mapdata, options, adapter) => {
    adapter.log.debug('creating map');
    if (options) {
    ...
    const canvas = createCanvas(1024 * 4, 1024 * 4);
    const ctx = canvas.getContext('2d');

    if (Mapdata.image.pixels.floor && Mapdata.image.pixels.floor.length !== 0) {
        if (typeof Mapdata.image.pixels.floor[0] === 'object') {
            //alte Valetudo version und Re bis 0.8
            ...
            if (Mapdata.image.pixels.floor && Mapdata.image.pixels.floor.length !== 0) {
                ...
                Mapdata.image.pixels.floor.forEach(function (coord) {
                    ctx.rect(
                        coord[0] * 4 + Mapdata.image.position.left * 4,
                        coord[1] * 4 + Mapdata.image.position.top * 4,
                        4,
                        4,
                    );
                });
        ...
        } else if (typeof Mapdata.image.pixels.floor[0] === 'number') {
            // neuere ValetudoRE und normale RR Map
            // init min
            minleft =
                (Mapdata.image.pixels.floor[0] % Mapdata.image.dimensions.width) * 4 + Mapdata.image.position.left * 4;
            mintop =
                (Mapdata.image.dimensions.height -
                    1 -
                    Math.floor(Mapdata.image.pixels.floor[0] / Mapdata.image.dimensions.width)) *
                    4 +
                Mapdata.image.position.top * 4;
            ['floor', 'obstacle'].forEach(key => {
                ...
                Mapdata.image.pixels[key].forEach(function drawPixel(px) {
                    ...
                    ctx.rect(
                        (px % Mapdata.image.dimensions.width) * 4 + Mapdata.image.position.left * 4,
                        (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                            Mapdata.image.position.top * 4,
                        4,
                        4,
                    );
        ...
        if (typeof Mapdata.currently_cleaned_zones[0] !== 'undefined') {
            ...
            Mapdata.currently_cleaned_zones.forEach(function (coord) {
                ...
                ctx.fillRect(
                    coord[0] / 12.5,
                    coord[1] / 12.5,
                    coord[2] / 12.5 - coord[0] / 12.5,
                    coord[3] / 12.5 - coord[1] / 12.5,
                );
    ...
    if (Mapdata && Mapdata.image && Mapdata.image.pixels && Mapdata.image.pixels.carpet) {
        ...
        Mapdata.image.pixels.carpet.forEach(px => {
            ...
            ctx.moveTo(
                (px % Mapdata.image.dimensions.width) * 4 + Mapdata.image.position.left * 4 + offset,
                (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                    Mapdata.image.position.top * 4,
            );
            ctx.lineTo(
                (px % Mapdata.image.dimensions.width) * 4 + Mapdata.image.position.left * 4 + offset + 4,
                (Mapdata.image.dimensions.height - 1 - Math.floor(px / Mapdata.image.dimensions.width)) * 4 +
                    Mapdata.image.position.top * 4 +
                    4,
            );
    ...
    if (Mapdata.robot) {
        ...
        if (
            Mapdata.path.current_angle &&
            typeof Mapdata.robot[0] !== 'undefined' &&
            typeof Mapdata.robot[1] !== 'undefined'
        ) {
            ctx.beginPath();
            ...
            ctx.drawImage(
                img,
                Mapdata.robot[0] / 12.5 - img.width / 2,
                Mapdata.robot[1] / 12.5 - img.height / 2,
                img.width,
                img.height,
            );
        }
    ...
    // crop image
    const finalHeight = maxtop - mintop + 2 * offset;
    const finalWidth = maxleft - minleft + 2 * offset;
    const canvas_final = createCanvas(finalWidth, finalHeight);
    ...
    const trimmed = ctx.getImageData(
        minleft - offset,
        mintop - offset,
        maxleft - minleft + 2 * offset,
        maxtop - mintop + offset,
    );
